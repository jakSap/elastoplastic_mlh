...
def is_gizmo_file(path):
    ...
    GIZMO writes .hdf5; the demonstrator writes .h5."""
    return str(path).endswith('.hdf5')


def read_record(h5File, withAngMom=False):
    """Return the conserved-quantity record for one snapshot in either format.

    The demonstrator stores the aggregates (totalMass, energy, xMomentum,
    yMomentum) directly. GIZMO snapshots store only per-particle fields, so we
    sum them here: total energy = kinetic + internal = sum_i m_i (1/2 |v_i|^2 + u_i),
    momentum = sum_i m_i v_i, mass = sum_i m_i. Velocities are taken in 2D to
    match the demonstrator's planar Kelvin-Helmholtz setup."""
    if is_gizmo_file(h5File):
        with h5.File(h5File, 'r') as data:
            m = data["PartType0/Masses"][:]
            v = data["PartType0/Velocities"][:, :2]
            u = data["PartType0/InternalEnergy"][:]
            t = float(data["Header"].attrs["Time"])
            rec = {
                "time":   t,
                "mass":   float(np.sum(m)),
                "energy": float(np.sum(m * (0.5 * (v[:, 0]**2 + v[:, 1]**2) + u))),
                "momX":   float(np.sum(m * v[:, 0])),
                "momY":   float(np.sum(m * v[:, 1])),
            }
            if withAngMom:
                x = data["PartType0/Coordinates"][:, :2]
                rec["angL"] = float(np.sum(m * (x[:, 0]*v[:, 1] - x[:, 1]*v[:, 0])))
        return rec
...
if __name__ == "__main__":
    ...
    def load_sim(directory, tstart=0.0, withAngMom=False):
        ...
        files = list(pathlib.Path(directory).glob('*.h5')) \
              + list(pathlib.Path(directory).glob('*.hdf5'))
        for h5File in files:
            records.append(read_record(h5File, withAngMom))
